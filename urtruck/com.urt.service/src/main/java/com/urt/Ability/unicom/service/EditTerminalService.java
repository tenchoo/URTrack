...
public class EditTerminalService extends UnicomWsService{
	...
	public SOAPMessage createRequest(Object... args) throws SOAPException,
		...
		if (args.length > 2) {
			targetValue = (String) args[2];
		}
		if (args.length > 3) {
			changeType = (String) args[3];
		} else {
			changeType = "4";
		}
		
		SOAPMessage message = getMessageFactory().createMessage();
		message.getMimeHeaders().addHeader("SOAPAction",SoapConstant.DOMAIN + SoapConstant.SOAPACTION_EDITTERMINAL);
        SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
        Name name = envelope.createName("EditTerminalRequest", SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
		...
		addElement(envelope, parentElement, "targetValue",targetValue, SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
		addElement(envelope, parentElement, "changeType",changeType, SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
		return message;
	}
	...
	public WsResponse parseMessage(SOAPMessage message) {
		try {
			...
			SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
	        Name parentName = envelope.createName("EditTerminalResponse", SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
	...
	public static void main(String[] args) throws Exception{
		UnicomWsService client = new EditTerminalService();
		client.init();
    	 ...
    	 * 
    	 */
//   	SOAPMessage soapMessage = client.call("89860616010010314686","lmh-test-EditTerminal","110WLW004085_MON-FIX_5120M-0S");
    	SOAPMessage soapMessage = client.call("89860616010010260707","10024551401","ACTIVATION_READY_NAME","3"); 
    	EditTerminalResponse response = (EditTerminalResponse) client.parseMessage(soapMessage);
