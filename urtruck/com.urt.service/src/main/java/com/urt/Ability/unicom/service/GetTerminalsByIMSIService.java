 ...
 * 
 */
@Service("getTerminalsByIMSIService")
public class GetTerminalsByIMSIService extends UnicomWsService {
	...
	public static void main(String[] args) throws SOAPException,
		...
		UnicomWsService client = new GetTerminalsByIMSIService();
		client.init();
    	...
    	list.add("460062901019713");
    	list.add("460062001088397");
		SOAPMessage soapMessage = client.call(list,"lmh-test-TerminalsByIMSI");
		GetTerminalsByImsiResponse response = (GetTerminalsByImsiResponse) client
	...
	SOAPMessage createRequest(Object... args) throws SOAPException,
		...
		message.getMimeHeaders().addHeader(
				"SOAPAction",
				SoapConstant.DOMAIN
						+ SoapConstant.SOAPACTION_GETTERMINALSBYIMSI);
		SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
		Name name = envelope.createName("GetTerminalsByIMSIRequest",
		...
		addElement(envelope, parentElement, "licenseKey", licenseKey,
				SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
		addElement(envelope, parentElement, "imsis", "imsi", imsis,
				SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
	...
	public WsResponse parseMessage(SOAPMessage message) {
		GetTerminalsByImsiResponse response = new GetTerminalsByImsiResponse();
		try {
			SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
			Name parentName = envelope.createName("GetTerminalsByIMSIResponse",
					SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
