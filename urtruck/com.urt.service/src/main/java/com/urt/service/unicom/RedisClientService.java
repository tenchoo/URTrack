...
public class RedisClientService {
	...
	public void  init() {
		if(pool == null) {
			...
			config.setSoftMinEvictableIdleTimeMillis(1800000);//毫秒，空闲多久，关闭该链接
			config.setMaxWaitMillis(Long.parseLong(maxWaitTime));//获取连接时的最大等待毫秒数
			config.setTestOnBorrow(true);
			pool = new JedisPool(config, host, Integer.parseInt(port), 100000);
		}
	}
	  ...
	  /**
	   * <p>通过key给field设置指定的值,如果key不存在则先创建,如果已经存在,返回0</p>
	   * @param key
	   * @param field
	   * @param value
	   * @return
	  ...
	  public Long expire(String key,int seconds){
	    ...
	    try {
	      jedis = pool.getResource();    
	      if((res = jedis.expire(key, seconds))!=1) return res;
	    } catch (Exception e) {      
	      e.printStackTrace();
	      return res;
	    } finally {
