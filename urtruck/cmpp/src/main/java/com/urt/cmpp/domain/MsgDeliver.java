	...
	public MsgDeliver(byte[] data){
		if(data.length>8+8+21+10+1+1+1+32+1+1+1+20){//+Msg_length+
		    ...
		    if(data[0] == 5 && data[1] == 0 && data[2] == 3){
		        System.out.println("收到一个长短信["+(data[3] & 0xFF)+"],共" + (data[4] & 0xFF) + "条，此条是第" + (data[5] & 0xFF) + "条");
		    }
			String fmtStr="utf-8";
			...
			try {
				...
				fmtStr=this.msg_Fmt==8?"UTF-16BE":"utf-8";
				 System.out.println("*************msg_Fmt**********"+msg_Fmt);
				...
				int size = getLength(b.toString());
				this.msg_Length=(byte)size;//消息长度
				byte[] msg_ContentByte=new byte[size];
				//---------------
				dins.read(msg_ContentByte);
				if(registered_Delivery==1){
					this.msg_Content=new String(msg_ContentByte,fmtStr);//消息长度
					//this.msg_Content=new String(msg_ContentByte,"UTF-8");//消息长度
