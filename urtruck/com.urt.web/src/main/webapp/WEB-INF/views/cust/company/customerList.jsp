...
<html>
<head>
<title><fmt:message bundle='${pageScope.bundle}'  key='Enterprise.customer.management' /></title>
...
<script type="text/javascript">
        ...
        function toDetail(id){
            if(id!=""){
                ...
                layer_show('<fmt:message bundle='${pageScope.bundle}'  key='customer.browse' />',url,'800','550');
                layerIndexs++;
            }
    ...
    var dataTableObj = {
        ...
        "sServerMethod":"post",
        "bServerSide": true,
        "sAjaxSource" : "<%=basePath%>cust/queryList", /* 跳转url */
                ...
                },
                {
                    ...
                    "mRender" : function(data, type, full,row) {
                        ...
                        return '<a title=\"<fmt:message bundle='${pageScope.bundle}'  key='Edit' />\" href="javaScript:toUpdate('
                                ...
                                + '<a title=\"<fmt:message bundle='${pageScope.bundle}'  key='Contact.person' />\"  style=\"text-decoration:none\" onclick=\"openConcatList('
                                + data.custId
                                + ');\"><fmt:message bundle='${pageScope.bundle}'  key='Contact.person' /></a>'/* data-toggle=\"modal\" data-target=\"#concatListModal\" */
                                + '<a title=\"<fmt:message bundle='${pageScope.bundle}'  key='Account.number' />\" class=\"ml-5\"  style=\"text-decoration:none\" onclick=\"openAccountList('
                                + data.custId + ');\"><fmt:message bundle='${pageScope.bundle}'  key='Account.number' /></a>';/* data-toggle=\"modal\" data-target=\"#concatListModal\" */
                    }
                }
...
<body>
    ...
    <div class="pd-20">
        ...
        <div class="fr mtb20">
            <a href="javascript:;" onclick="add_cust('<fmt:message bundle='${pageScope.bundle}'  key='add.customers' />','${ctx}/cust/company/createCustomer','800','550')" class="btn btn-primary radius">
            <span class="human"></span><fmt:message bundle='${pageScope.bundle}'  key='add.customers' /></a>
            <a href="javascript:;" onclick="import_cust()" class="btn btn-primary radius">
            <!-- <i class="Hui-iconfont">&#xe600;</i> -->
                    <fmt:message bundle='${pageScope.bundle}'  key='import.customers' /></a>
        </div>
        <div class="mt-20">
            <table id="example" class="table table-border table-bordered table-hover table-bg table-sort " cellpadding="0">
                <thead>
                    <tr class="zpTable">
                        ...
                        <th><fmt:message bundle='${pageScope.bundle}'  key='Customer.state' /></th>
                        <th><fmt:message bundle='${pageScope.bundle}'  key='Document.type' /></th>
                        <th><fmt:message bundle='${pageScope.bundle}'  key='Document.number' /></th>
