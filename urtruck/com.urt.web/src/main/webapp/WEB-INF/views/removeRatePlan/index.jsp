...
<html>
<head>
<title><fmt:message bundle='${pageScope.bundle}'  key='Batch.delete.traffic' /></title>
<base href="<%=basePath%>" />
...
<script type="text/javascript">
	function upload(){
		var option = {
			url : "<%=basePath%>removeRatePlan/sendRemoveRatePlanMsg",
			type : "post",
			success : function(data) {
				...
				for ( var key in Data) {
					...
					if (key.indexOf('total') > -1) {
						$("#example").append(
								"<tr class='text-c'><td><fmt:message bundle='${pageScope.bundle}'  key='Number.of.successful.in.batch.processing' /></td><td>"
										+ Data[key] + "</td></tr>");
					} else if (key.indexOf('errorMsg') > -1) {
						$("#example").append(
								"<tr class='text-c'><td><fmt:message bundle='${pageScope.bundle}'  key='Error.message' /></td><td>"
										+ Data[key] + "</td></tr>");
					}
				}
				$("#showResult").hide();
...
<body>
	<div class="pd-20 font12">
		<form role="form" action="/removeRatePlan/sendRemoveRatePlanMsg"
