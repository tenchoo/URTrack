...
import com.urt.storm.batchFtpImport.BatchFtpImportBolt;
import com.urt.storm.util.ZookeeperClient;

public class ReorderSpoutAndTupo {
	// 汇报offset信息的root路径
	private static final String offsetZkRoot = "/archiveZkRoot";
	// 存储该spout id的消费offset信息,譬如以topoName来命名
	private static final String offsetZkId = "reorderToppic";
	// topicName
	private static final String TOPICS = "reorderToppic";
	...
	static {
		...
		try {
			...
			STARTOFFSETTIME = Long.parseLong(prop
					.getProperty("startOffsetTime").trim());
	...
	protected static void submitTopologyRemoteCluster(String arg,
		...
		config.put(Config.TOPOLOGY_MAX_SPOUT_PENDING, 200);// 设置一个spout
															// task上面最多有多少个没有处理的tuple（没有ack/failed）回复，以防止tuple队列爆掉
	...
	public static SpoutConfig getSpoutConfig() {
		...
		SpoutConfig spoutConfig = new SpoutConfig(brokerHosts, TOPICS,
				offsetZkRoot, offsetZkId);
	...
	public static StormTopology getTopolgyKafkaSpout() {
		...
		tBuilder.setSpout("reorderSpout",
				new KafkaSpout(getSpoutConfig()), 3);
		tBuilder.setBolt("reorderBolt", new ReorderBolt(), 15)
				.setNumTasks(10).shuffleGrouping("reorderSpout");
