...
public class RemoveRatePlanKafkaSpoutTopologyMain {
	//汇报offset信息的root路径
		private static final String offsetZkRoot = "/archiveZkRoot";
	    //存储该spout id的消费offset信息,譬如以topoName来命名
		private static final String offsetZkId = "removeRatePlanTopic";
		//topicName
	    private static final String TOPICS = "removeRatePlanTopic";
	    ...
	    protected static void submitTopologyRemoteCluster(String arg, StormTopology topology, Config config) throws Exception {
	    	...
	    	config.put(Config.TOPOLOGY_ACKER_EXECUTORS, 14);//设置acker的数量 
	    	config.put(Config.TOPOLOGY_MAX_SPOUT_PENDING, 248);//设置一个spout task上面最多有多少个没有处理的tuple（没有ack/failed）回复，以防止tuple队列爆掉 
	    ...
	    public static StormTopology getTopolgyKafkaSpout() {
	        ...
	        tBuilder.setSpout("RemoveRatePlanSpout", new KafkaSpout(getSpoutConfig()),2);
	        tBuilder.setBolt("RemoveRatePlanBolt", new RemoveRatePlanBolt(),15).setNumTasks(10).shuffleGrouping("RemoveRatePlanSpout");
