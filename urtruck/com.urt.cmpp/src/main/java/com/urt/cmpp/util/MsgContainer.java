...
import com.urt.cmpp.domain.MsgConnect;
import com.urt.cmpp.domain.MsgHead;
import com.urt.cmpp.domain.MsgSubmit;
...
//				}//*/
	...
	public static  boolean sendMsg(String msg,String cusMsisdn, String lang){
		try{
			if(msg.getBytes("utf-8").length<140){//短短信
				boolean result=sendShortMsg(msg, cusMsisdn, lang);
//				int count=0;
//				while(!result){
//					count++;
//					
...
//				result=sendShortMsg(msg,cusMsisdn);
				...
				boolean result=sendLongMsg(msg ,cusMsisdn, lang);
				int count=0;
				System.out.println("long result:"+result);
				while(!result){
					count++;
					result=sendLongMsg(msg, cusMsisdn, lang);
					if(count>=(MsgConfig.getConnectCount()-1)){//如果再次连接次数超过两次则终止连接
						break;
					}
				}
	 ...
	 * @return
	 */
	private static boolean sendShortMsg(String msg, String cusMsisdn, String lang){
		try {			
			int seq=MsgUtils.getSequence();
			MsgSubmit submit=new MsgSubmit();
			
			if("EngLish".equals(lang)){
				submit.setTotalLength(12+8+1+1+1+1+10+1+32+1+1+1+1+6+2+6+17+17+21+1+32+1+1+msg.length()+20);
				submit.setMsgContent(msg.getBytes("UTF8"));
				submit.setMsgFmt((byte)0x00);
			...
			submit.setRegisteredDelivery((byte)0x1);
			...
			submit.setServiceId("DDTX");
			List<byte[]> dataList=new ArrayList<byte[]>();
			dataList.add(submit.toByteArry(lang));
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("懂得通信于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发短短信，序列号为:"+seq);
			return true;
		} catch (Exception e) {
			try {
				out.close();
	 ...
	 * @return
	 */
	private static boolean sendLongMsg(String msg, String cusMsisdn, String lang){
		try {
			byte[] allByte=msg.getBytes("UTF8");
			List<byte[]> dataList=new ArrayList<byte[]>();
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("懂得通信于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发长短信，序列号为:"+seqId);
			return true;
		} catch (Exception e) {
			try {
				out.close();
	...
	private static boolean sendShortWapPushMsg(String url,String desc,String cusMsisdn){
		try {
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("懂得通信于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发web push短短信，序列号为:"+seq);
			return true;
		} catch (Exception e) {
			try {
				out.close();
	...
	private static boolean sendLongWapPushMsg(String url,String desc,String cusMsisdn){
		try {
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("懂得通信于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发web pus长短信，序列号为:"+seqId);
			return true;
		} catch (Exception e) {
			try {
				out.close();
