 ...
 * 
 */
@Service("getModifiedSMSService")
public class GetModifiedSMSService extends UnicomWsService {
	...
	public static void main(String[] args) throws SOAPException,
		...
		UnicomWsService client = new GetModifiedSMSService();
		client.init();
	...
	SOAPMessage createRequest(Object... args) throws SOAPException,
		...
		if (args.length > 2) {
			fromDate = (String) args[2];
		}
		if (args.length > 3) {
			toDate = (String) args[3];
		}
		...
		message.getMimeHeaders().addHeader("SOAPAction",
				SoapConstant.DOMAIN + SoapConstant.SOAPACTION_GETMODIFIEDSMS);
		SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
		Name name = envelope.createName("GetModifiedSMSRequest",
		...
		addElement(envelope, parentElement, "iccid", iccid,
				SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
		addElement(envelope, parentElement, "fromDate", fromDate,
				SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
		addElement(envelope, parentElement, "toDate", toDate,
				SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
	...
	public WsResponse parseMessage(SOAPMessage message) {
		GetModifiedSMSResponse response = new GetModifiedSMSResponse();
		try {
			SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
			Name parentName = envelope.createName("GetModifiedSMSResponse",
					SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
			...
			List<Long> smsMsgIds = new ArrayList<Long>();
				...
				for (int i = 0; i < list.getLength(); i++) {
					...
					for (int j = 0; j < childList.getLength(); j++) {
						...
						if ("smsMsgId".equalsIgnoreCase(name)) {
							smsMsgIds.add(Long.parseLong(value));
						}
					}
				}
			}
			response.setSmsMsgIds(smsMsgIds);
