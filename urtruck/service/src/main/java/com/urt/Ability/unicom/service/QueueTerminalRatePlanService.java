 ...
 *
 */
@Service("QueueTerminalRatePlanService")
public class QueueTerminalRatePlanService extends UnicomWsService{
	...
	SOAPMessage createRequest(Object... args) throws SOAPException,
		...
		message.getMimeHeaders().addHeader("SOAPAction",SoapConstant.DOMAIN + SoapConstant.SOAPACTION_QUEUETERMINALRATEPLAN);
        SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
        Name name = envelope.createName("QueueTerminalRatePlanRequest", SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
	...
	public WsResponse parseMessage(SOAPMessage message) {
		try {
			QueueTerminalRatePlanResponse response = new QueueTerminalRatePlanResponse();
			SOAPEnvelope envelope = message.getSOAPPart().getEnvelope();
	        Name parentName = envelope.createName("QueueTerminalRatePlanResponse", SoapConstant.PREFIX, SoapConstant.NAMESPACE_URI);
...
//		System.setProperty("javax.net.ssl.trustStore", "jssecacerts");
	...
	public static void main(String[] args) throws Exception{
		UnicomWsService client = new QueueTerminalRatePlanService();
		client.init();
    	 ...
    	 * 
    	 */
    	String[] dates={"89860617020000292486"};
    	for (int i = 0; i < dates.length; i++) {
			
//    		SOAPMessage soapMessage = client.call("89860616010010313845","lmh-test-EditTerminal","110WLW004085_PRE-IND_5M");
    		SOAPMessage soapMessage1 = client.call(dates[i],"lmh-test-EditTerminal","110WLW004085_PRE-IND_1024M");
    		SOAPMessage soapMessage2 = client.call(dates[i],"lmh-test-EditTerminal","110WLW004085_MON-FIX_5120M-0S");
    		QueueTerminalRatePlanResponse response1 = (QueueTerminalRatePlanResponse) client.parseMessage(soapMessage1);
    		QueueTerminalRatePlanResponse response2 = (QueueTerminalRatePlanResponse) client.parseMessage(soapMessage2);
    		System.out.println("序号"+i+"-----"+response1);
    		System.out.println("序号"+i+"-----"+response2);
		}
