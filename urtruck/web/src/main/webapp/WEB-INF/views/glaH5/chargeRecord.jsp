...
<body>
	<div style="font-size: 3.8vw;overflow: hidden;">
		<div  style="background-color: #fa9148; border-bottom: 1px solid #dfe4e1; margin: 0; overflow: hidden;"
			...
			<div style="color: #ffffff;margin: 1.25rem 3.8rem;font-size:4.2vw;" class="col-xs-4 "><fmt:message bundle='${pageScope.bundle}'  key='Recharg.records' /></div>
		</div>
								...
								<div style="padding: 2rem 0 0.2rem 0; margin: 0 auto 0rem 0rem"
									...
									<div style="margin: 0.5rem auto 0 auto" class="row">
										...
										<div
											...
											class="col-xs-2">
											<a 
											href="<%=basePath%>glaH5AppQuery/queryChargeRecordDetail?iccid=${item.iccid}&username=${item.username}&orderid=${item.orderid}&flowsize=${item.flowsize}&payamount=${item.payamount}&paytype=${item.paytype}&chargedate=<fmt:formatDate value='${item.chargedate}' pattern='yyyy-MM-dd HH:mm:ss' />">
					...
					<c:if test="${not empty tradeList}">
						<c:forEach items="${tradeList}" var="item">
							<div
								...
								<div style="padding: 1rem 0 0.5rem 0; margin: 0.5rem auto 0 0"
									class="col-xs-2">
									<img src="${item.goodsDto.goodsPic}" style="height: 5rem">
								</div>
								<div style="padding: 2rem 0 0.2rem 0; margin: 0 auto 0rem 0rem"
									class="col-xs-10 ">
									<div style="margin: 0.5rem auto 0 auto" class="row">
										<div	
											style="padding-right: 0; margin-right: 0; text-align: right"
											class="col-xs-5">
											<h5>${item.goodsDto.goodsName}<fmt:message bundle='${pageScope.bundle}'  key='data.plan' /></h5>
										</div>
										<div style="padding-right: 0; text-align: center"
											class="col-xs-5">
											<h5 style="padding-right: 0;">${item.fee}<fmt:message bundle='${pageScope.bundle}'  key='yuan' /></h5>
										</div>
										<div
											style="margin-left: 0rem; padding-right: 0; text-align: center"
											class="col-xs-2">
											<a 
											href="<%=basePath%>glaH5AppQuery/queryChargeRecordDetail?orderid=${item.tradeId}&flowsize=${item.goodsDto.goodsName}&payamount=${item.fee}&paytype=${item.payType}&chargedate=<fmt:formatDate value='${item.payDate}' pattern='yyyy-MM-dd HH:mm:ss' />">
											<img src="static/h5/images/right.png" style="height: 1.5rem">
											</a>
										</div>
									</div>
			...
			<div style="margin-bottom: 3rem;width:100vm;padding-left:0;padding-right:0" class="row">
				<div style="margin-top: 2rem; text-align: center"
					...
					<a style="font-size: 3.8vw"
						href="<%=basePath%>glaH5AppQuery/queryChargeRecord?curPage=${curPage-1 < 1? curPage:(curPage-1)}&startTime=${startTime}&endTime=${endTime}"}><fmt:message bundle='${pageScope.bundle}'  key='The.previous.page' /></a>
				</div>
				...
				<div style="margin-top: 2rem; text-align: center" class="col-xs-4">
					<a style="font-size: 3.8vw"
						href="<%=basePath%>glaH5AppQuery/queryChargeRecord?curPage=${curPage+1 > totalPage? curPage:(curPage+1)}&startTime=${startTime}&endTime=${endTime}"}><fmt:message bundle='${pageScope.bundle}'  key='The.next.page' /></a>
				</div>
			</div>
		</div>
