...
<html>
<head>
	<title><fmt:message bundle='${pageScope.bundle}'  key='Product.release.display' /></title>
...
<link rel="stylesheet" type="text/css"
	...
	<script type="text/javascript">
		...
		function toAddAlarmRule(){
			url='${ctx}/alarm/createRule';
			layer_show('<fmt:message bundle="${pageScope.bundle}"  key="edit.warning.rules" />',url,'800','550');
		}
		//更新
		function toUpdate(goodsReleaseId){
			if(goodsReleaseId!=""){
				var url = '${ctx}/goodsRelease/toUpdateGoodsRelease?goodsReleaseId='+goodsReleaseId;
				layer_show('<fmt:message bundle="${pageScope.bundle}"  key="Modify.released.products" />',url,'800','550');
			}

			
		}
		
		function toAddGoodsRelease(title,url,w,h){
		...
		function del(goodsReleaseId){
			if(confirm("<fmt:message bundle='${pageScope.bundle}'  key='are.you.sure.that.you.want.to.delete.it' />")){
					if(goodsReleaseId!=""){							
						$.ajax({
					        type:"POST",
					        url:"${ctx}/goodsRelease/delGoodsRelease",
		...
		//设置产品失效
		function toUneffective(goodsReleaseId){
		    //alert(goodsReleaseId)
			$.ajax({
		        type:"POST",
		        url:"${ctx}/goodsRelease/toUneffective",
		        data:{'goodsReleaseId':goodsReleaseId},
				dataType : "json",
				success : function(data) {
					layer.msg("<fmt:message bundle='${pageScope.bundle}'  key='Uneffectived' />");
					location.reload();
				},
				error : function(error) {
				}
	...
	var dataTableObj = {
		...
		"sServerMethod":"post",
	    "bServerSide": true,
		"sAjaxSource" : "<%=basePath %>goodsRelease/list",    /* 跳转url */
		"iDisplayLength": 5,  /* 展示条数 */	
			         ...
			         {"targets": [11],"data": null,
			        	 "mRender": function ( data, type, full ) {
			                	return'<a title=\"<fmt:message bundle='${pageScope.bundle}'  key="Uneffective" />\" href="javaScript:toUneffective('
								+ data.goodsReleaseId
								+ ');" class=\"ml-5\" style=\"text-decoration:none\"><fmt:message bundle='${pageScope.bundle}'  key="Uneffective" /></i></a> '
...
<body>
	...
	<form role="form" >
	    ...
	    <div class="col-md-4 col-lg-3 mt20">
      	...
      	<div style="margin-left:50px;"  class="col-md-4 col-lg-3 mt20">
			<label for="name" class="font12 fl"><fmt:message bundle='${pageScope.bundle}'  key='Expiration.status' />：</label>
			<div class="tBox">
				<select id="activeStatus" name="activeStatus" class="input-text">
					<option value="0"><fmt:message bundle='${pageScope.bundle}'  key='Effectived' /></option>
					<option value="1"><fmt:message bundle='${pageScope.bundle}'  key='Uneffectived' /></option>
				</select>
			</div>
      	</div>
      	<div class="row">
...
<div class="fr pd-10" style="float: left;"><span class="l"><a title="产品发布" href="javaScript:toAddGoodsRelease();" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>产品发布</a></span></div>		
 --%>			<div class="fr pd-10">
			<a href="javascript:;" onclick="toAddGoodsRelease('<fmt:message bundle='${pageScope.bundle}'  key='Release.products' />','${ctx}/goodsRelease/toAddGoodRelease','800','550')" class="btn btn-primary radius">
			<!-- <i class="Hui-iconfont">&#xe600;</i> --><span class="human"></span>
					<fmt:message bundle='${pageScope.bundle}'  key='Product.release' /></a>
		</div>
	<div class="mt-20">
