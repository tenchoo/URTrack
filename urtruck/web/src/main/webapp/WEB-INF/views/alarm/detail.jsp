...
<title><fmt:message bundle='${pageScope.bundle}'  key='view.warning.rules' /></title>
			...
			<form method="post" class="form form-horizontal" id="form_rule"
				...
				<div class="row cl">
					<div class="col-10 from-control">
						<div class="formControls col-8 col-offset-2">
							<input type="text" class="input-text" id="ruleName" name="ruleName" required value="${ruleDto.ruleName}" readonly="readonly">
				...
				<div class="row cl">
					<div class="col-10 from-control">
						...
						<div class="formControls col-3">
							<input type="hidden" id="level2Id" value="${ruleDto.ruleTypeId}" />
				...
				if(condition!=null && condition!="undefinde"){
					...
					for(var i=0; i<condition.length; i++){
						...
						if(condition[i].DISPLAY_TYPE==1){
							$("#"+condition_div).append("<label class='form-label col-3'>"+condition[i].ELEM_DISPLAY+"</label>");
						}
						if(condition[i].DISPLAY_TYPE==2){
							debugger;
							var textValue=elemValue[condition[i].ELEMENT_ID]=='undefined'?'':elemValue[condition[i].ELEMENT_ID];
							$("#"+condition_div).append("<div class='formControls col-7'><input type='text' readonly='readonly' class='input-text' value='"+textValue+"' name='"+condition[i].ELEMENT_ID+"' required />"
						...
						if(condition[i].DISPLAY_TYPE==3){
							...
							$("#"+condition_div).append("<select id='"+condition[i].ELEMENT_ID+"' disabled='disabled' name='"+condition[i].ELEMENT_ID+"' placeholder='<fmt:message bundle='${pageScope.bundle}'  key='Please.choose' />..' class='form-control select2'></select>"
									+"<input type='hidden' name='elementIds' value='"+condition[i].ELEMENT_ID+"'></div>");
						    ...
						    if(elemValue[condition[i].ELEMENT_ID]!='undefined'){
								selectName.val(elemValue[condition[i].ELEMENT_ID]).trigger('change');
						    }
				...
				if(execute!=null && execute!="undefinde"){
					...
					for(var i=0; i<execute.length; i++){
						...
						if(execute[i].DISPLAY_TYPE==1){
							$("#"+execute_div).append("<label class='form-label col-3'>"+execute[i].ELEM_DISPLAY+"</label>");
							/* $("#condition").append("<div id="+condition_div+" class='col-10 from-control'>"); */
						}
						if(execute[i].DISPLAY_TYPE==2){
							var textValue=elemValue[execute[i].ELEMENT_ID]=='undefined'?'':elemValue[execute[i].ELEMENT_ID];
							$("#"+execute_div).append("<div class='formControls col-7'><input type='text' readonly='readonly' class='input-text' value='"+textValue+"' name='"+execute[i].ELEMENT_ID+"' required />"+
						...
						if(execute[i].DISPLAY_TYPE==3){
							...
							"<select id='"+execute[i].ELEMENT_ID+"' name='"+execute[i].ELEMENT_ID+"' placeholder='<fmt:message bundle="${pageScope.bundle}"  key="Please.choose" />..' disabled='disabled' class='form-control select2'></select>"+
							"<input type='hidden' name='elementIds' value='"+execute[i].ELEMENT_ID+"'></div>");
							debugger; 
							var selectName="sel_"+execute[i].ELEMENT_ID;
							 ...
							 if(elemValue[execute[i].ELEMENT_ID]!='undefined'){
								 selectName.val(elemValue[execute[i].ELEMENT_ID]).trigger('change');
							 }
