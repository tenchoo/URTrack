 ...
 */
public class NotifyOperationUtils {
	private static final Log logger = LogFactory.getLog(NotifyOperationUtils.class);
		...
		try {
			...
			if(rowNum > 0){
				...
				for (int i = 1; i <= rowNum; i++) {
					...
					while (j < colNum) {
						resultMap.put(titleRow.getCell(j).toString().trim(), getStringCellValue(row.getCell(j)).trim());
						j++;
					}
	...
	public static void main(String[] args) {
		NotifyOperationUtils utils = new NotifyOperationUtils();
